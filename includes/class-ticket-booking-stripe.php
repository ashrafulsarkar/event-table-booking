...
class Ticket_Booking_Stripe {
		...
		try {
			...
			$wpdb->insert(
				...
				[ 
					...
					'payment_method'   => sanitize_text_field( $pay_method ),
					'payment_status'   => 'Confirmed',
					'company_location' => sanitize_text_field( $location ),
					'bin_number'       => sanitize_text_field( $bin_number ),
					'total_amount'     => floatval( $total_amount / 100 ),
					'total_vat'        => floatval( $total_vat / 100 ),
					'vat_percentage'   => intval( $vatPercentage ),
					'payment_id'       => sanitize_text_field( $payment_intent->id ),
					'order_id'         => sanitize_text_field( $order_id ),
				],
				[ '%s', '%s', '%s', '%s', '%s', '%s', '%d', '%s', '%s', '%s', '%s', '%f', '%f', '%d', '%s', '%s' ]
			);
	...
	public function payment_return_handler() {
		if ( isset( $_GET['payment_intent'] ) ) {
			...
			$results = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $table_name WHERE order_id = %s", $order_id ) );

			if ( $results ) {
				return '<div class="payment-success"><i class="fa-regular fa-circle-check"></i><h2>Thank you!</h2><h4>Your booking has been confirmed.</h4><div class="payment-details"><div class="booking"><p>Booking number: </p><p><b>' . esc_html( $results->order_id ) . '</b></p></div><div class="order"><p>Order date: </p><p><b>' . date( "M j, Y", strtotime( $results->order_date ) ) . '</b></p></div><p class="send_mail">We have sent detailed information about the order confirmation to your email <b>' . esc_html( $results->email ) . '</b></p></div></div>';
			} else {
